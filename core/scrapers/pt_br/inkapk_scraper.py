...
class InkapkScraper(BaseScraper):
    """Scraper para o site InkApk."""
    ...
    def get_chapters(self, series_url):
        ...
        chapter_items = soup.select('.ink-ch-list .ink-ch-item')
    ...
    def get_chapter_images(self, chapter_url):
        ...
        # As imagens estão dentro de div.reading-content > img
        images = []
        for img in soup.select('.reading-content img'):
            src = img.get('data-src') or img.get('src')
            if src:
                src = src.strip()
                # Se não começar com http (por exemplo /wp-content/...), completa a URL
                if not src.startswith('http'):
                    src = f"https://inkapk.net{src}"
                images.append(src)
